...
def find_stim_pairs(stim_dir: Path):
    """
    stim_01_r.png / stim_01_nr.png のペアを番号順に列挙
    """
    ...
    pairs = [StimPair(i, r_map[i], nr_map[i]) for i in idxs]
    return pairs
...
def run(args):
    ...
    try:
        ...
        if not stim_pairs:
            raise RuntimeError(f"{stim_dir} に stim_XX_r.png / stim_XX_nr.png のペアが見つかりません")
        ...
        out_path = out_dir / f"{args.name}touch_2stim_log_{start_dt.strftime('%Y%m%d_%H%M%S')}.csv"
        ...
        def place_new_trial():
            ...
            if current_trial_is_correction and (correction_left_is_r is not None):
                left_is_r = correction_left_is_r   # 前試行と同一配置を維持
            else:
                left_is_r = bool(random.getrandbits(1))  # 毎試行ランダム
...
def parse_args():
    ...
    # 刺激
    p.add_argument("--stim-dir", type=str, required=True, help="stim_XX_r.png と stim_XX_nr.png が入ったフォルダ")
    ...
    p.add_argument("--plate-rgb", type=int, nargs=3, default=(150,150,150), help="プレート色 (R G B)")
    # 配置（中心から等距離）
