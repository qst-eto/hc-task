...
def run(args):
    ...
    try:
        ...
        def draw(stim_on: bool):
            ...
            if args.info:
                txt1 = f"State={['SHOW','ITI','WAIT_RELEASE'][state]}  Rect {rect.w}x{rect.h} (mode={mode}, frac={size_frac:.3f})  successes={successes}"
                screen.blit(pygame.font.SysFont(None, 28).render(txt1, True, (220, 220, 220)), (20, 20))
            pygame.display.flip()

        def append_log(event_name, x, y, iti_ms):
        ...
        while running:
            ...
            for ev in pygame.event.get():
                ...
                if state == STATE_SHOW:
                    ...
                    if ev.type in (pygame.MOUSEBUTTONDOWN, pygame.FINGERDOWN):
                        ...
                        if ok:
                            ...
                            if shrink_every > 0 and (successes % shrink_every == 0):
                                # 最小サイズ下限（割合×短辺 と min_side_px の大きい方）を保証
                    ...
                    # ここでは記録を求められていないので、何もしない（離し判定のみ）
                    pass
        ...
        with out_path.open("w", newline="", encoding="utf-8") as f:
            fieldnames = ["start_iso","iso","rel_s","state","x","y","rect_w","rect_h","iti_ms","event"]
