...
# Rect layout
# =========================
def compute_rect(sw, sh, mode, size_frac, square_px, fullscreen):
    short = min(sw, sh)
    if mode == "auto":
        side = max(1, int(short * size_frac))
        if fullscreen and side >= short:
            return pygame.Rect(0, 0, sw, sh)  # 全面
        x = (sw - side)//2; y = (sh - side)//2
        return pygame.Rect(x, y, side, side)
    if mode == "square_custom":
        if square_px is None: raise ValueError("--square-px が必要です")
        side = max(1, int(square_px))
        x = (sw - side)//2; y = (sh - side)//2
        return pygame.Rect(x, y, side, side)
    # 'square'
    side = max(1, int(short * size_frac))
...
def run(args):
    ...
    try:
        ...
        def cur_rect():
            return compute_rect(sw, sh, mode, size_frac, square_px, args.fullscreen)
        rect = cur_rect()
        ...
        def draw(stim_on: bool):
            ...
            if args.info:
                txt1 = f"State={['SHOW','ITI','WAIT_RELEASE'][state]}  Rect {rect.w}x{rect.h} (mode={mode}, frac={size_frac:.3f})"
        ...
        running = True
        iti_end_time = 0.0  # for STATE_ITI
                ...
                if state == STATE_SHOW:
                    ...
                    if ev.type in (pygame.MOUSEBUTTONDOWN, pygame.FINGERDOWN):
                        ...
                        logs.append({
                            ...
                            "x": x, "y": y, "rect_w": rect.w, "rect_h": rect.h,
                            "iti_ms": iti_ms, "event": "TOUCH_TTL" if ok else "TOUCH_TTL_FAIL"
                        })
                    ...
                    # ここでもブランク。release待ちだけ（下のループ終端で判定）
                    pass
...
def parse_args():
    p = argparse.ArgumentParser(description="Touchscreen rectangle training (FSM: SHOW→ITI→WAIT_RELEASE)")
    # 画面
