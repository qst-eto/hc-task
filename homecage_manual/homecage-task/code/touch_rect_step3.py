# touch_rect_v4_outside_fail_random.py
...
def _place_square_random(sw: int, sh: int, side: int, fullscreen: bool, allow_auto_fullscreen: bool) -> pygame.Rect:
    ...
    - 上下はマージン m を取り、m = min(side/2, (sh - side)/2)
      （= 可能なら side/2、不可能なら取り得る最大値）
    - auto_fullscreen 条件成立時は全面矩形にする
    - はみ出しは絶対に起こさないよう side を画面内にクランプ
    """
    side = max(1, int(side))
    short = min(sw, sh)
    ...
    x = random.randint(x_min, x_max) if x_max >= x_min else 0  # 念のため

    # 上下（垂直）は「矩形サイズ/2」のマージン要求。ただし実現不可能なら最大値まで。
    desired_m = side // 2
    max_feasible_m = max(0, (sh - side) // 2)  # m ≤ (sh - side)/2 が可否条件
    m = min(desired_m, max_feasible_m)

    y_min, y_max = m, sh - side - m
    # m を上のように定義しているので必ず y_max >= y_min のはずだが、保険で分岐
...
def compute_rect(sw, sh, mode, size_frac, square_px, fullscreen, min_side_px, allow_auto_fullscreen):
    """
    以前は中央固定だったが、サイズ決定後にランダム配置（上下は side/2 マージン）。
    """
    short = min(sw, sh)
    ...
    if mode == "auto":
        ...
        return _place_square_random(sw, sh, side, fullscreen, allow_auto_fullscreen)
...
def run(args):
    ...
    try:
        ...
        mouse_down = False
        active_fingers = set()
        outside_touches_in_trial = 0  # 試行内カウンタ

        # ITI関連フラグ
        touch_during_iti = False  # ITI区間中に一度でもタッチがあったか
        require_release_dwell = False  # 次試行前に連続解放を要求するか
        ...
        # 旧バージョン互換の誤記対応（読み取りのみ）: sqaure_custom -> square_custom
        if args.rect_mode == "sqaure_custom" and args.square_px is not None:
            rect = compute_rect(sw, sh, "square_custom", size_frac, args.square_px, args.fullscreen, min_side_px, allow_auto_fs)
        ...
        while running:
            ...
            for ev in pygame.event.get():
                ...
                # ---- State machine ----
                if state == STATE_SHOW:
                    # 刺激表示中：矩形内→成功（TTL/ビープ/ITI=correct）
                    #             矩形外→カウント、上限到達で失敗/ITI=error
                    ...
                    # ここでは離し判定のみ（下の「State transitions」で継続的に評価）
                    pass
        ...
        with out_path.open("w", newline="", encoding="utf-8") as f:
            fieldnames = [
                "start_iso","iso","rel_s","state","x","y","rect_w","rect_h",
                "iti_ms","event","outside_in_trial","max_outside_before_fail",
